...
(function () {
    ...
    module.directive('dirDisqus', ['$window', '$location', function ($window, $location) {
        return {
            restrict: 'E',
            scope: {
                config: '='
            ...
            link: function (scope) {
                ...
                function configChanged() {
                    ...
                    if (!scope.config || !scope.config.disqus_shortname || !scope.config.disqus_identifier || !scope.config.disqus_url) {
                        return;
                    }
                    scope.host = $location.protocol() + "://" + $location.host();
                    scope.button = scope.host + '/static/img/logo.png';
                    scope.login = scope.host + '/simplelogin';
                    scope.logout = scope.host + '/logout';
                    ...
                    $window.disqus_config = function () {
                        ...
                        this.sso = {
                            name: scope.config.disqus_identifier,
                            button: scope.button,
                            url: scope.login,
                            logout: scope.logout,
                            width: "800",
